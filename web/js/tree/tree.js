  ...
  this.toString = function()
  {
    ...
    if (this.useArrow)  //使用方向键控制跳转到上级下级父级子级节点
    {
      if (document.attachEvent)
		...
		//load("TreeViewFocus" + TreeView.getFileName());	// 获取以前保存在userdate中的状态
		var nodeSid = getAttribute("TreeItemSid");	// 滚动条左位置
		TreeView.focus(nodeSid)
	}
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(!isRoot)
  {
    ...
    if(this.wordLine)
    {
      HTML += "<SPAN aaaa>"+ node.parentNode.childAppend + (AtEnd ? "└" : "├") +"</SPAN>";
  ...
  HTML = "\r\n<SPAN id='"+ this.name +"_tree_"+ id +"' sid='TreeItemSID_"+node.sid+"'>"+ HTML
...
MzTreeView.prototype.getFileName = function(){
...
MzTreeView.prototype.focusClientNode      = function(id)
{
  ...
  //实现单击方法。add by LCK
  try{
	var index = a.id.substring(a.id.lastIndexOf("_")+1)
	var nodeSid = document.getElementById(TreeView.name +"_tree_"+ index).sid
	with(window.document.body){
		setAttribute("TreeItemSid",nodeSid.substring(nodeSid.indexOf("_")+1));
		save("TreeViewFocus" + this.getFileName());
	}
	clickTreeItem(a);
  }catch(ex){
  }
...
/**
	初始化加载所有的树节点（影响速度。但是有时用得上：模块授权）
*/
MzTreeView.prototype.LoadAllNode = function(){
	this.expandAll(true);
}
MzTreeView.prototype.expandAll1 = function()
{
  if(this.totalNode>500) 
   // if(confirm("您是否要停止展开全部节点？\r\n\r\n节点过多！展开很耗时")) return;
  if(!confirm("Are you sure to expand all nodes?  It will spend more times"))return;
...
//展开树的所有节点
MzTreeView.prototype.expandAll = function(SureInitAll)//参数只有LoadAllNode才会出现
{
  if(this.totalNode>500)
  	//if(confirm("您是否要停止展开全部节点？\r\n\r\n节点过多！展开很耗时")) return;
  	if(!confirm("Are you sure to expand all nodes?  It will spend more times"))return;
...
//收缩树的所有节点
MzTreeView.prototype.backExpandAll = function()
{
  if(this.totalNode>500)
  	//if(confirm("您是否要停止展开全部节点？\r\n\r\n节点过多！展开很耗时")) return;
  	//if(!confirm("Are you sure to expand all nodes?  It will spend more times"))return;
