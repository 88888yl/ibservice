 ...
 */
public class ExportCSOServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        response.setHeader("Content-disposition", "attachment; filename=export-cso-" + dateStr + ".xls");
        ServletOutputStream os = response.getOutputStream();

        ExportToExcel exportToExcel = new ExportToExcel(
                GlobalVariables.oracleUrl, GlobalVariables.oracleUserName, GlobalVariables.oraclePassword);
        exportToExcel.setStore(store);
        List<List<String>> rows = exportToExcel.getRowsFromStr("TOTAL_CSO");
