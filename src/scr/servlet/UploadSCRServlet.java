 ...
 */
public class UploadSCRServlet extends HttpServlet {
    String defaultFileName = "SCR";
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (item.isFormField()) {
                    ...
                    if (item.getName() != null && !item.getName().equals("") && item.getName().contains(defaultFileName)) {
                        System.out.println("Upload file-----name----:" + item.getName());
                        System.out.println("Upload file-----size----:" + item.getSize());
                        File file = new File(GlobalVariables.scrPath + item.getName());
                        item.write(file);
